...
namespace Adam4017
{
    public class Adam4017 : IAdam4017
    {
        ...
        /// <inheritdoc/>
        public async Task<List<decimal>> ReadSignalValueAsync(string address = "01", int timeOut = -1)
        {
            return (await _crowPort.RequestAsync(new ReadSignalValueReq(address), new Func<byte[], ReadSignalValueRsp>(rspByte => new ReadSignalValueRsp(rspByte)), timeOut, true)).RecData;
        }
    }
}